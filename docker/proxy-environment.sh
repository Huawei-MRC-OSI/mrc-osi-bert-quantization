...
if test -z "$https_proxy" ; then
  echo "http_proxy and https_proxy are not set. Skipping proxy-environment setup" >&2
  exit 0
fi
...
if test -d "$JAVA_HOME" && test -f "$cacerts" ; then
  ...
  echo "JAVA_HOME is not set or '$cacerts' file is not installed. Skipping Java certificates setup." >&2
fi
...
# TODO: Fixup for TensorFlow build, unchecked. If not works, move
#       `/etc/bazelrc` to `$HOME/.bazelrc`
cat >/etc/bazelrc <<EOF
